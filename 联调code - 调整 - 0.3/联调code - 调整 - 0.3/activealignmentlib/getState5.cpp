...
HV_Lap getEdgeLaplace(Mat img, int block_size, float thr, int edge_size, int error, double f, double scale, double pixel_size, double d_obj, double size, double block,int * Laplace_Flag);
...
State getState5(Mat img_1, Mat img_2, Mat img_3, Mat img_4, Mat img_5, double pixel_size, double scale)
{
	...
	for (int i = 0; i < 5; i++){
		hv_lap.push_back(getEdgeLaplace(img[i], block_size, thr, edge_size, error, f, scale, pixel_size, d_obj, size, block,&Lap_flag));
