...
BOOL JpegFile::SaveJpeg(CString fileName,BYTE * pData,UINT nWidth,UINT nHeight)
{
	...
	bool color = true;
    BYTE* pTemp = new BYTE [nWidth*3*nHeight];
	if(pTemp==NULL)
	{
		AfxMessageBox("memory is over!");
		return false;
	}
	memcpy(pTemp,pData,nWidth*3*nHeight);

	// we vertical flip for display. undo that.
//	JpegFile::VertFlipBuf(pTemp, nWidth*3, nHeight);

	// we swap red and blue for display, undo that.
	JpegFile::BGRFromRGB(pTemp, nWidth, nHeight);
...
BYTE* JpegFile::LoadJpeg(CString fileName, UINT *nWidth, UINT *nHeight)
{
	...
	pTemp=JpegFile::JpegFileToRGB(fileName, &width, &height);
	...
	JpegFile::BGRFromRGB(pTemp, width, height);
	JpegFile::VertFlipBuf(pTemp, width * 3, height);
	BYTE *tmp=NULL;
    UINT widthDW = 0;    
		// DWORD-align for display
	tmp = JpegFile::MakeDwordAlignedBuf(pTemp,
										 width,
										 height,
										 &widthDW);
	delete []pTemp;
