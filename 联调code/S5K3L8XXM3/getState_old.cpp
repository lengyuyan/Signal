...
State getState(Mat img_ori, Mat img_p, Mat img_m, double deltaZ_p, double deltaZ_m, double pixel_size,double scale,int cameraID,int * flag)
{
	...
	cout << endl << "Delta_Z：" ;
	vector<double> Getangle_x = getangle(x_lap_o, x_lap_p, x_lap_m, x_pt, deltaZ_p, deltaZ_m, g_AAParam.pixel_size/g_AAParam.scale);
	...
	vector<double> Getangle_y = getangle(y_lap_o, y_lap_p, y_lap_m, y_pt, deltaZ_p, deltaZ_m, g_AAParam.pixel_size/g_AAParam.scale);
