...
#pragma comment (lib,"IAVisionAlgorithm.lib") //算法库
	...
	, m_FocusPos(0)
{
	...
	ReadAA_Globle_Pic_ParamValue();//读取图像配置文件
	...
	{	
		dlgAARobotProcessEvent[i] = CreateEventA(NULL, false, false, NULL);	//不是手动触发,初始状态为false
		
		AARobotWakeResult[i] = CreateEventA(NULL, false, false, NULL);	//不是手动触发,初始状态为false
		AARobotProcessResult[i].result = -1;//结果是-1;
		AARobotProcessParam[i] = -1;

		//复位结果事件
		AARobotResetAAWakeResult[i] = CreateEventA(NULL, false, false, NULL);	//不是手动触发,初始状态为false
	}
...
void CHSFoxxDlg::DoDataExchange(CDataExchange* pDX)
{
	...
	DDX_Control(pDX, IDC_SPIN_CENTER, m_spincenter);
...
BEGIN_MESSAGE_MAP(CHSFoxxDlg, CDialog)
	...
	//双击事件 单击事件预留画图
	ON_STN_DBLCLK(IDC_VIDEO0, &CHSFoxxDlg::OnStnClickedVideo0)
	ON_STN_DBLCLK(IDC_VIDEO1, &CHSFoxxDlg::OnStnClickedVideo1)
	ON_STN_DBLCLK(IDC_VIDEO2, &CHSFoxxDlg::OnStnClickedVideo2)
	ON_STN_DBLCLK(IDC_VIDEO3, &CHSFoxxDlg::OnStnClickedVideo3)
	ON_STN_DBLCLK(IDC_VIDEO4, &CHSFoxxDlg::OnStnClickedVideo4)
	ON_STN_DBLCLK(IDC_VIDEO5, &CHSFoxxDlg::OnStnClickedVideo5)
	ON_STN_DBLCLK(IDC_VIDEO6, &CHSFoxxDlg::OnStnClickedVideo6)
	ON_STN_DBLCLK(IDC_VIDEO7, &CHSFoxxDlg::OnStnClickedVideo7)

	ON_BN_CLICKED(IDC_CONNECTCLOSE, &CHSFoxxDlg::OnBnClickedConnectclose)//连接关闭
	...
	ON_WM_HSCROLL()
	ON_BN_CLICKED(IDC_SPIN_CENTER, &CHSFoxxDlg::OnBnClickedSpinCenter)
	ON_WM_MOUSEMOVE()

	ON_BN_CLICKED(IDC_DRAWCENTERPOINT, &CHSFoxxDlg::OnBnClickedDrawcenterpoint)
END_MESSAGE_MAP()
...
//////////////////////////////////////////////////////
	...
	//设置增益 曝光滚动条
	m_grain.SetRange(0, AAVISION_MAXGRAIN);
	m_exp.SetRange(0, AAVISION_MAXEXPURE);
...
void CHSFoxxDlg::OnPlay() 
{
	...
	if (it != mapVision.end())
	{
		if (m_nVisionDevID>=0 && m_nVisionDevID != it->second->m_iCurIDA)
		{
		...
		m_nVisionDevID = it->second->m_iCurIDA;
	}
	else
	{
	...
	{	
		...
		if(DT_ERROR_OK == IsDevConnect(m_nVisionDevID))  //if old device is online,close it...
		{
			msg("vision(%s)设备已经打开！",strTemp.c_str());

			//根据m_nDevID来传递不同的hwnd;
			HWND h = GetHWndByVisionId(m_nVisionDevID);

			RECT videosize ;
			m_wndVideo[vision->m_iDevIDA].GetWindowRect(&videosize);
			int width = videosize.right - videosize.left;
			int height = videosize.bottom - videosize.top;
			if((ret = vision->OpenVisionVideo(h,m_pictureFlag[vision->m_iDevIDA],width,height)) != DT_ERROR_OK)
			{
...
BOOL CHSFoxxDlg::bOpenDevice()
{
	...
	if(m_nVisionDevID != m_KitListCtrl.GetCurSel())
	{
		if (m_nVisionDevID >= 0)
		{
			if(DT_ERROR_OK == IsDevConnect(m_nVisionDevID))  //if old device is online,close it...
			{
				CloseDevice(m_nVisionDevID);
			}
		}
	}
	else if (m_nVisionDevID >= 0)
	{
		if(DT_ERROR_OK == IsDevConnect(m_nVisionDevID))  //if current device is online ,returan directly...
		{
			return TRUE;
		}
	}
...
BOOL CHSFoxxDlg::bCloseCamera()
{
	...
	if(1)
	{
		ResetSensorI2cBus(m_nVisionDevID);
		SensorEnable(CurrentSensor.pin ^ 2, 1, m_nVisionDevID);       //reset sensor....
		Sleep(2);
		CloseVideo(m_nVisionDevID);
		
		SensorEnable(CurrentSensor.pin ^ 1, 1, m_nVisionDevID);       //power down sensor....
	}
	//close all power....
	{
		//close the clock
		SetSensorClock(0, 24*10, m_nVisionDevID);
		//close the IO power...
		SetSoftPinPullUp(IO_NOPULL, m_nVisionDevID);
		//close reset and power down pin...
		SensorEnable(0, 1, m_nVisionDevID);
		...
		PmuSetVoltage(Power, Volt, 5, m_nVisionDevID);
		Sleep(50);
		PmuSetOnOff(Power,OnOff,5,m_nVisionDevID);
...
{
	DWORD dwKitType = (GetKitType(m_nVisionDevID)&0xFF);
	if(dwKitType  < 0x90)
		return -1;
...
//20130703 changed....
	...
	OS_Config((int)(1.5f*1000*1000), HighLimit, LowLimit, 45, 2*1000, 1*1000, m_nVisionDevID);	
	//OS 测试，进行该PIN为low的测试。
	//command bit7 :enalble os ; bit3:high test; bit2:low test...
	LC_OS_CommandConfig(0x80 | 0x08 | 0x04, IoMask, 10, m_nVisionDevID);
	//读取OS测试的结果
	if(DT_ERROR_OK == OS_Read(VoltageH, VoltageL, Result, 1, 1, 45 , m_nVisionDevID))
	{
...
void CHSFoxxDlg::GetADValue()
{
	...
	Power[3] = POWER_AFVCC;
	Power[4] = POWER_VPP;
	PmuGetCurrent(Power, Current, 5, m_nVisionDevID);
...
LPBYTE CHSFoxxDlg::GetPicture(int caramID)
{
...
BOOL CHSFoxxDlg::GetPictureMat(Mat * PictureMat,int caramID)
{
	...
	int lineByte = (vision->m_PreviewWidthA + 3)/4*4;

	//应为要用opencv的mat来计算,所以这里把LPBYTE 直接构造成了MAT，注意第一个参数为高，也就是行数
	Mat img(vision->m_PreviewHeightA,vision->m_PreviewWidthA,CV_8UC3,BmpBuffer);
	//Mat img(vision->m_PreviewHeightA,vision->m_PreviewWidthA,CV_8UC3,BmpBuffer,lineByte);
	//BmpBuffer = img.data;
	*PictureMat=img.clone();	
	pictureRows = PictureMat->rows;
	pictureCols = PictureMat->cols;
	free(BmpBuffer);
	return TRUE;
}

BOOL CHSFoxxDlg::GetPictureHObject(HObject &PictureHobject,int caramID)
{
	LPBYTE BmpBuffer = NULL;
	CAAVision *vision = NULL;
	map<string,CAAVision*>::iterator it;
	...
	IAVH_ImgByte2HObject(PictureHobject, BmpBuffer,vision->m_PreviewWidthA,vision->m_PreviewHeightA,1);
	free(BmpBuffer);
	return TRUE;
}
...
LPBYTE CHSFoxxDlg::AAVisionGrabPicture(int visionID)
{
	...
	for (it = mapVision.begin();it != mapVision.end();it++)
	{
		if (it->second->m_iCurIDA == visionID )
		{
			vision =  it->second;
			break;
		}
...
void CHSFoxxDlg::OnBnClickedSavepicture()
{
	...
	for (it = mapVision.begin();it != mapVision.end();it++)
	{
		if (it->second->m_iDevIDA == m_nVisionDevID)
		{
			vision = it->second;
			break;
		}
	...
	if (vision == NULL )
	{
		msg("当前视觉不存在,视觉id:%d",m_nVisionDevID);
		return ;
	}


	BmpBuffer = AAVisionGrabPicture(m_nVisionDevID);

	if (BmpBuffer== NULL)
	{
		msg("获取图像失败,视觉id:%d",m_nVisionDevID);
		return  ;
	}
	//获得了图片指针，把他保存起来
	...
	sfilename.Format("P%02d%02d%02d%02d%02d_%s_%d.jpg",time.GetMonth(),time.GetDay(),time.GetHour(),
		time.GetMinute(),time.GetSecond(),vision->m_pSensorNameA.c_str(),m_nVisionDevID);

	int ret = IAVH_SavePictureInByte(BmpBuffer,sfilename.GetBuffer(),vision->m_PreviewWidthA,vision->m_PreviewHeightA,3);
	if (0 != ret)
	{
		MessageBox("保存图片失败!!");
	}
...
void CHSFoxxDlg::OnBnClickedSetpower()
{
	...
	//设置5路电压值
	if (PmuSetVoltage(Power,Volt,5,m_nVisionDevID) != DT_ERROR_OK)
	{
...
void CHSFoxxDlg::OnBnClickedSetmclk()
{
	...
	if (m_nVisionDevID >= 0 != NULL)
	{
		SetSensorClock(TRUE,(USHORT)(m_fMclk*10),m_nVisionDevID);
	}
}
...
void CHSFoxxDlg::OnBnClickedFocusPos()
{
	...
	if(m_RunMode != RUNMODE_STOP)
	{
		...
		if(FindAfDevice(m_nVisionDevID))
		{
			WriteAfValue(m_FocusPos << 4, 0, m_nVisionDevID);
			AfxMessageBox(stmp);
		}
	}
}
int CHSFoxxDlg::AdjustZvalue(int robotID,int cameraID)
{
	...
	while(!(( fabs(AAadjust_z)<zadjust_zthr && distanceToOriginal[temp_number]< 0.02)||(temp_number>=z_adjustnum)))
	{
		...
		if(Plus_flag != TRUE)
		{
			msg("获取距离为:%.6f图片失败,调整次数:%d",distanceToOriginal[temp_number],temp_number);
		...
		if(Minus_flag != TRUE)
		{
			msg("获取负向图片失败");return AA_GET_PICTIRE_FAIL;
		}
		if(AARobotMoveRelative(robotID,axisString, &(distanceToOriginal[temp_number]))<0)
		{
	 		msg("采集完3张图片后,机器返回初始位置失败");return AA_GO_RERATIVEPT_FAIL;
		}
	    //step2.计算移动的距离.
		...
		int flag = 0;
		flag = calZUV(img_ori,img_p,img_m,distanceToOriginal[temp_number],-distanceToOriginal[temp_number],cameraID,state);
		double adjust_z = state.deltaZ; //
	    //step3..根据计算结果进行移动..
		const char posStr[16] = "Z U V";
		double pos[3] ={0.0};
		if(state.OneSide_flag[0]==0&&state.OneSide_flag[1]==0)
		{
			pos[0]=adjust_z;
		}
		else if(abs(state.OneSide_flag[0])==1||abs(state.OneSide_flag[1])==1)
		{
			pos[0]=adjust_z;
			if (state.OneSide_flag[0]!=0)
			msg("(robotid:%d,visonid:%d)x中心单边情况!继续调整",robotID,cameraID);
			if (state.OneSide_flag[1]!=0)
			msg("(robotid:%d,visonid:%d)y中心单边情况!继续调整",robotID,cameraID);
		}
		else
		{
...
int CHSFoxxDlg::AdjustUVvalue2(int robotID,int cameraID)
{
	...
	int temp_number=0;
	int count = 0;
	double distanceToOriginalUV[20]={0.0};
	Mat Ori_PictureMat;
	Mat Plus_PictureMat;
	Mat Minus_PictureMat;
	int OneSide_flag = 0;//判断单边情况
	const char axisString[5]="Z ";
	...
	//配置文件里只有10项，暂时把后面10项都置为0.07
	for (int i = 10;i<20;i++)
	{
		distanceToOriginalUV[i] = 0.007;
	}
	

	while(!(( fabs(AAadjust_u)<adjust_uvthr && fabs(AAadjust_v)<adjust_uvthr  && distanceToOriginalUV[temp_number]< 0.01)||(count >= 20)||(temp_number >= 19)))
	{
		...
		int Plus_flag=GetPictureMat(& Plus_PictureMat,m_nVisionDevID);
		if(Plus_flag != TRUE)
		{
			msg("获取正向原始图片失败,返回！");
			return AA_GET_PICTIRE_FAIL;
		...
		int Minus_flag=GetPictureMat(& Minus_PictureMat,m_nVisionDevID);
		if(Minus_flag != TRUE)
		{
			msg("获取负向原始图片失败,返回！");
			return AA_GET_PICTIRE_FAIL;
		...
		{
			msg("(robotid:%d,visonid:%d)采集3张图片后,机器返回初始位置失败",robotID,cameraID);
		...
		//	cameraID,&OneSide_flag);
		int flag = 0;
		flag = calZUV(Ori_PictureMat,Plus_PictureMat,Minus_PictureMat,distanceToOriginalUV[temp_number],-distanceToOriginalUV[temp_number],cameraID,state);
		...
		if(state.OneSide_flag[2]==0 && state.OneSide_flag[3]==0)
		{
			pos[1]=adjust_u;
			//pos[2]=adjust_v;
		}
		else if(abs(state.OneSide_flag[2])==1 || abs(state.OneSide_flag[3])==1)
		{
			pos[1]=adjust_u;
			//pos[2]=adjust_v;
			if (state.OneSide_flag[2]!=0)
			msg("(robotid:%d,visonid:%d)u计算,左边出现%d单边,继续!",robotID,cameraID,state.OneSide_flag[2]);
			if (state.OneSide_flag[3]!=0)
			msg("(robotid:%d,visonid:%d)u计算,右边出现%d单边,继续!",robotID,cameraID,state.OneSide_flag[3]);
		}
		else
		{
		...
		if(state.OneSide_flag[4]==0 && state.OneSide_flag[5]==0)
		{
			//pos[1]=adjust_u;
			pos[2]=adjust_v;
		}
		else if(abs(state.OneSide_flag[4])==1 || abs(state.OneSide_flag[5])==1)
		{
			//pos[1]=adjust_u;//上下是对应u??
			pos[2]=adjust_v;
			if (state.OneSide_flag[4]!=0)
			msg("(robotid:%d,visonid:%d)v计算,上边出现%d单边,继续!",robotID,cameraID,state.OneSide_flag[4]);
			if (state.OneSide_flag[5]!=0)
			msg("(robotid:%d,visonid:%d)v计算,下边出现%d单边,继续!",robotID,cameraID,state.OneSide_flag[5]);
		}
		else
		{
			msg("(robotid:%d,visonid:%d)调整时,初始位置误差范围太大,或者找不到点,返回!OneSide_flag:%d,",robotID,cameraID,OneSide_flag);
		...
		if (!((state.OneSide_flag[2]==1 && state.OneSide_flag[3]==1) || (state.OneSide_flag[4]==1 && state.OneSide_flag[5]==1)||
			(state.OneSide_flag[2]==-1 && state.OneSide_flag[3]==-1) || (state.OneSide_flag[4]==-1 && state.OneSide_flag[5]==-1)))
		{
		    int result=AARobotMoveRelative(robotID,posStr, pos);
		    if(result<0)
		    {
		...
		{
			count++;
			//if (temp_number < uv_adjustnum-1)
			//temp_number--;
			//if (temp_number < 0) temp_number =0;
			//msg("(robotid:%d,visonid:%d)uv轴无调整",robotID,cameraID);

			
			if ((state.OneSide_flag[2]==-1 && state.OneSide_flag[3]==-1)||(state.OneSide_flag[4]==-1 && state.OneSide_flag[5]==-1))
				pos[0] = -distanceToOriginalUV[temp_number];//adjust_z;
			if ((state.OneSide_flag[2]==1 && state.OneSide_flag[3]==1)||(state.OneSide_flag[4]==1 && state.OneSide_flag[5]==1))
				pos[0] = distanceToOriginalUV[temp_number];//adjust_z;
			pos[1] = 0;
			pos[2] = 0; 
			const char posStrU[16] = "Z";
		    int result_z=AARobotMoveRelative(robotID,posStrU, pos);
			msg("(robotid:%d,visonid:%d)z轴调整:%.6lf",robotID,cameraID,pos[0]);
		    if(result_z<0)
		    {
			    msg("(robotid:%d,visonid:%d)z移动为:%.6lf,超出机器运动范围,继续",robotID,cameraID,pos[0]);
			    //temp_number++;
			//continue;
	    	}
			temp_number--;
	...
	msg("(robotid:%d,visonid:%d)调整uv轴完毕！调整次数:%d",robotID,cameraID,count);
	return count;
...
int CHSFoxxDlg::FiveAdjustvalue(int robotID,int cameraID)
{
	...
	int Plus_flag=GetPictureMat(& Plus_PictureMat1,m_nVisionDevID);
	if(Plus_flag != TRUE)
	{
		msg("获取正向原始图片失败");
		return AA_GET_PICTIRE_FAIL;
...
//输入参数为，灰度阈值的范围(最小值，最大值) 从ROI区域中找到的Region区域面积的最小值，最大值
void CHSFoxxDlg::CalcCenterPoint(int vaid, int vbid, HObject &pictureA, HObject &pictureB, int graymin,int graymax,int areamin,int areamax)
{
	HObject OutObject;
	double center[2] = {0.0};
	IAVH_PARAM_STRUCT param;
	param.selectShape.min = g_AA_PicParam.selectShape[vaid].min; // areamin
	param.selectShape.max = g_AA_PicParam.selectShape[vaid].max; // areamax
	param.scaleImage.mul = g_AA_PicParam.scaleImage[vaid].mul;
	param.scaleImage.add = g_AA_PicParam.scaleImage[vaid].add;

	//每幅图片上的5个ROI区域，区域按照上.下，中，左,右来排列方式
	IAVH_ROI_STRUCT roiP[2][AA_VISION_RETAGLELOC];
	//获取ROI矩形坐标
	for( int j=0; j<AA_VISION_RETAGLELOC;j++ )
	{
		roiP[0][j].rect.Row1 = g_AAParam.channel1Retangle[vaid][j].ltx;
		roiP[0][j].rect.Column1 = g_AAParam.channel1Retangle[vaid][j].lty;
		roiP[0][j].rect.Row2 = g_AAParam.channel1Retangle[vaid][j].rbx;
		roiP[0][j].rect.Column2 = g_AAParam.channel1Retangle[vaid][j].rby;
	}
	for( int j=0; j<AA_VISION_RETAGLELOC;j++ )
	{
		roiP[1][j].rect.Row1 = g_AAParam.channel1Retangle[vbid][j].ltx;
		roiP[1][j].rect.Column1 = g_AAParam.channel1Retangle[vbid][j].lty;
		roiP[1][j].rect.Row2 = g_AAParam.channel1Retangle[vbid][j].rbx;
		roiP[1][j].rect.Column2 = g_AAParam.channel1Retangle[vbid][j].rby;
	}

	for( int j=0; j<AA_VISION_RETAGLELOC;j++ )
	{
		IAVH_GetImgRetangleROI(pictureA,OutObject, roiP[0][j],0);

		//计算指定区域内的特征点
		IAVH_GetImgROICenterByArea(OutObject,center,param,0);

		coordinate[vaid][j].x = center[1];
		coordinate[vaid][j].y = center[0];
	}
	for( int j=0; j<AA_VISION_RETAGLELOC;j++ )
	{
		IAVH_GetImgRetangleROI(pictureB,OutObject, roiP[1][j],0);
		//计算指定区域内的特征点
		IAVH_GetImgROICenterByArea(OutObject,center,param,0);
		coordinate[vbid][j].x = center[1];
		coordinate[vbid][j].y = center[0];
	}
}

//新的双摄调整: 分析两个摄像头的图像，返回偏差值
AANotifyResult CHSFoxxDlg::NewStartDoubleAA(int vaid,int vbid)
{
	//策略: 得到偏差值,然后根据偏差值是否符合精度(w,u,v<=0.1),但是每次都计算3个值
	//策略一:	1,运动控制发指令过来：同时调整w,u,v：isfinsh = 0;
	//			2，如w,u,v大于精度阈值时，每次把大于阈值的分开为一半分别赋值给两个模组，小于阈值时，赋值为0;
	//			3,当w,u,v <=0.1时，isfinsh =1

	//策略二:	1,运动控制发指令过来：这边首先调整w,返回指令：isfinsh = 0; u,v=0.0
	//			2,当w<=0.1时，一起调整UV; 也就是说根据w来调整UV
	//			3,当u,v<=0.1

	//注意这里设置w,u,v3个精度阈值，以及一个调整次数阈值，若分开调整，每个维度的都最大调节次数为这么多, 设置策略配置
	//暂时按照策略1来
	AANotifyResult result ;
	result.isfinsh = 0;
	result.result = 1;
	int ret=0;
	Para param = {0};
	unsigned char * pBmpA = NULL;
	unsigned char * pBmpB = NULL;
	HObject A,B,filterA,filterB;
	g_AAParam.AdjuestMethod =1;

	switch(g_AAParam.AdjuestMethod)
	{
	case 1: //策略1
		{
			if( vaid <0 || vbid<0 )
			{
				msg("没有连接。visonidfixID：%d,visonidmovID：%d.",vaid,vbid);
				result.result = 0;
				break;
			}
			pBmpA =	GetPicture(vaid);
			if(pBmpA == NULL)
			{
				//异常
				free(pBmpA);
			...
			ret = IAVH_ImgByte2HObject(A,pBmpA,g_AAParam.pic_lenth,g_AAParam.pic_width);
			
			pBmpB =	GetPicture(vbid);
			if(pBmpB == NULL)
			{
				//异常
				free(pBmpA);
			...
			ret = IAVH_ImgByte2HObject(B, pBmpB,g_AAParam.pic_lenth,g_AAParam.pic_width);
			//中值滤波
			ret = IAVH_Median_Filter(A, "square", 7, "continued", filterA);
			ret = IAVH_Median_Filter(B, "square", 7, "continued", filterB);
			if( ret != 0 )
			{
				//异常
				free(pBmpA);
			...
			//得到 w u v 
			param = calUVW(vaid,vbid,filterA,filterB);
			free(pBmpA);
			free(pBmpB);
			pBmpA = NULL;
			pBmpB = NULL;
			if (abs(param.u) <= g_AAParam.doubleAAUth && abs(param.v) <= g_AAParam.doubleAAVth && abs(param.w) <= g_AAParam.doubleAAWth)
			{
				msg("需要调整范围:u:%.6lf,v:%.6lf,w:%.6lf,在精度范围内,完成调整!",param.u,param.v,param.w);
				result.isfinsh =1;
				break;
			}
			else
			{
				result.tha[0] = param.u;
				result.tha[1] = param.v;
				result.tha[2] = param.w;

				result.thb[0] = param.u;
				result.thb[1] = param.v;
				result.thb[2] = param.w;
			}
			msg("机器移动u:%.6lf,v:%.6lf,w:%.6lf",param.u,param.v,param.w);	
			////机器人移动
			...
			//movpos[1] = param.v;
			//movpos[2] = param.w;
			//ret = AARobotMoveRelative(0,axisString, movpos);
			//if(ret <0)
			//{
			//	msg("机器移动失败!");
			//	break;
	...
	case 2:             
		{
			...
			free(pBmpB);
			//中值滤波
			HObject filterA,filterB;
			IAVH_Median_Filter(A, "square", 9, "continued", filterA);
			IAVH_Median_Filter(B, "square", 9, "continued", filterB);
			//得到 w u v 
			param = calUVW(vaid,vbid,filterA,filterB);
			if( param.w < g_AAParam.doubleAAUth )
			{
				param = calUVW(vaid,vbid,filterA,filterB);
				if( param.u < g_AAParam.doubleAAUth && param.v < g_AAParam.doubleAAUth )
				{
					result.isfinsh = 0;
				}
				else
...
int CHSFoxxDlg::StartDoubleAA(int robotid,int visonidfixID,int visonidmovID)
{
	...
	Para param = {0};
	int count = 0;
	int countth = 15; //测试次数阈值
	double thu = 0.1,thv =0.1,thw=0.1;
	if (visonidfixID <0 || visonidmovID<0 || robotid <0)
	{
		...
		return AA_BASE_ERROR_CODE;
	}
	unsigned char * pBmpA = NULL;
	unsigned char * pBmpB = NULL;
	HObject A,B;

	//算法说明:最终结果是u,v ,w的旋转角度都在设定的精度范围内(默认是0.1℃)，那计算时首先判断是否在精度范围内
	//在范围内不需要旋转,不在范围内则旋转
	msg("阈值:%.6lf,v:%.6lf,w:%.6lf",thu,thv,thw);
	int a=1;
	while (a)
	{	
		pBmpA =	GetPicture(visonidfixID);
		if(pBmpA == NULL)
		{
			msg("采图失败!,id:%d",visonidfixID);
			return AA_GET_PICTIRE_FAIL;
		}	
		ret = IAVH_ImgByte2HObject(A, pBmpA,g_AAParam.pic_lenth,g_AAParam.pic_width);
		...
		free(pBmpA);
		pBmpA = NULL;
		pBmpB =	GetPicture(visonidmovID);
		if(pBmpB == NULL)
		{
			
			msg("采图失败!,id:%d",visonidmovID);
			return AA_GET_PICTIRE_FAIL;
		}
		ret = IAVH_ImgByte2HObject(B, pBmpB,g_AAParam.pic_lenth,g_AAParam.pic_width);
		//IAVH_ShowPicture(B);
		//IAVH_ShowPicture(A);
		/*ret = IAVH_SavePicture(A,".//iamge//fixh.jpg");
		ret = IAVH_SavePicture(B,".//image//movh.jpg");*/
		//中值滤波
		HObject filterA,filterB;
		IAVH_Median_Filter(A, "square", 9, "continued", filterA);
		IAVH_Median_Filter(B, "square", 9, "continued", filterB);
		...
		double center[2] = {0.0};
		param = calUVW(visonidfixID,visonidmovID,filterA,filterB);
		free(pBmpA);
		free(pBmpB);
		...
		////第二种方法,角点法
		//param = calPara3(fixMat, movMat, g_AAParam.doubleAAdis, g_AAParam.pixel_size/g_AAParam.doubleAAscale, g_AAParam.focus, g_AAParam.objdis);	
		movpos[0] = param.u;
		movpos[1] = param.v;
		movpos[2] = param.w;
		//保存 U V W 的值到 csv文件
		if (!CFG_INSTANCE->is_file_exist("data\\doubleUVW.csv"))
		{
			CFG_INSTANCE->write_file("data\\doubleUVW.csv","参数U,参数V,参数W \n");
		}
		CFG_INSTANCE->write_file("data\\doubleUVW.csv","%7lf,%.7lf,%.7lf \n",movpos[0],movpos[1],movpos[2]);

		if (abs(movpos[0]) <= thu && abs(movpos[1]) <= thv && abs(movpos[2]) <= thw)
		{
			ret = AA_OK;
			msg("需要调整范围:%.6lf,v:%.6lf,w:%.6lf,在精度范围内,完成调整!",movpos[0],movpos[1],movpos[2]);
			break;
		}
		if (count >=countth)
		{
			msg("需要调整范围:%.6lf,v:%.6lf,w:%.6lf,调整超出限制%d次!",movpos[0],movpos[1],movpos[2],countth);
			break;
		}
		count++;
		msg("机器移动u:%.6lf,v:%.6lf,w:%.6lf",movpos[0],movpos[1],movpos[2]);
		ret = AARobotMoveRelative(robotid,axisString, movpos);
		if(ret <0)
		{
			msg("机器移动失败!");
			ret = AA_BASE_ERROR_CODE;
			break;
		}
		else
		{
	...
	ret = IAVH_ImgByte2HObject(B, pBmpB,g_AAParam.pic_lenth,g_AAParam.pic_width);
	IAVH_SavePicture(B,".//iamge//movh后.jpg");
	
	free(pBmpB);
	return ret;
}
int CHSFoxxDlg::StartDoubleAAByOpenCV(int robotid,int visonidfixID,int visonidmovID)
{
	//步骤:抓两个图像,然后以visonidfixID为基准来调节visonidmovID
	//绑定关系为:(Camera_1,Camera_2),(Camera_3,Camera_4),(Camera_5,Camera_6),(Camera_7,Camera_8)
	int ret = AA_OK;
	...
	while (a)
	{
		...
		savePicture(movMat,"movMatA.jpg");
		savePicture(fixMat,"fixMatA.jpg");
		//缩小图小,提高速度
		fixMat = Img_resize(fixMat, g_AAParam.doubleAAscale);
		movMat = Img_resize(movMat, g_AAParam.doubleAAscale);
		//第一种方法,求质心法，给出u,v,w的方法,黑标靶
...
void CHSFoxxDlg::OnBnClickedStartaa()
{
	...
	if (m_nPiDevID<0 || m_nVisionDevID <0)
	{
		msg("没有连接,返回!当前机器人id:%d,视觉id:%d",m_nPiDevID,m_nVisionDevID);
		//return;
	}
	CreateDirectory(".\\image",NULL);//创建文件夹
	 if (g_AAParam.AATYPE == DOUBLEAA)//双摄像头处理
	 {
		 ...
		 { 
			 if (1 == g_AAParam.IsRobot)
			 {
				//步骤:抓两个图像,然后以visonidfixID为基准来调节visonidmovID,首先确保固定的通道已经调整好
				 //绑定关系为:(Camera_1,Camera_2),(Camera_3,Camera_4),(Camera_5,Camera_6),(Camera_7,Camera_8)
				int fixedvisionID= FindBinderIDByID(m_nVisionDevID);
				//ret =   StartDoubleAA(m_nPiDevID,fixedvisionID,m_nDevID);
				//ret =   StartDoubleAAByOpenCV(m_nPiDevID,1,0);

				ret =   StartDoubleAA(m_nPiDevID,0,1);
			 }
			 else
			 {
				 AANotifyResult res;
				 //测试用途
				 int vaid = 0,vbid = 1;
				 res = NewStartDoubleAA(vaid,vbid); 
	...
	{
		SavePicture(".\\image\\"+FirstImage+"_First.jpg",m_nVisionDevID);
	}	
	double time = static_cast<double>(getTickCount());	
	//一，调整z
	double distanceToOriginal[10]={0};
	int  first_z_number = AdjustZvalue(m_nPiDevID,m_nVisionDevID);
	if (first_z_number<0)
	{
		return ;
	}
	...
	//二：然后需再调整U/V轴
	adjust_uv_number = AdjustUVvalue2(m_nPiDevID,m_nVisionDevID);
	if (adjust_uv_number<0)
	{
		return ;
	}
	...
	if (CFG_INSTANCE->get_cfg_int(AACoreCfg,savepicture))
	{
 		SavePicture(".\\image\\"+FirstImage+".jpg",m_nVisionDevID);//_Last
	}
	return;
...
void CHSFoxxDlg::ChangeVideoWind(int index )
{
	CAAVision* vision = FindVisionByID(index);
	if (NULL != vision )
	{
		HWND h = GetHWndByVisionId(vision->m_iCurIDA);
		//测试窗口绑定：
		RECT videosize ;
		m_wndVideo[vision->m_iCurIDA].GetWindowRect(&videosize);
		int width = videosize.right - videosize.left;
		int height = videosize.bottom - videosize.top;
		int ret =0;
		if((ret = vision->ChangeVideoWind(h,m_pictureFlag[vision->m_iCurIDA],width,height)) != DT_ERROR_OK)
		{
			//msg("vision(%s)视频打开失败！",vision->);
			//bt.SetCheck(FALSE);
			return ;
		}
	}
...
void CHSFoxxDlg::MoveOffset(int iAxis, int iFlag)
{
	...
	if (m_spincenter.GetCheck() == TRUE)//旋转中心调整
	{
		int result = AdjustSpinCenter(m_nPiDevID,iAxis,Parray);
		if (TRUE != result)
		{
			msg("设置旋转中心失败，请打开PI软件查看，重新设置!");
		}
...
vector<AANotifyResult> WINAPI CHSFoxxDlg::WorkNotify(int evt, AANotifyData data, void* lparam)
{
	CHSFoxxDlg* pDlg = (CHSFoxxDlg*)lparam;
	vector<AANotifyResult>result;
	
	if (1 == g_AAParam.IsRobot && DAA == evt ) //机器人内测
	{
		evt = STARTAA;//只有调整的时候会用到机器人和运动控制卡的不同
	}
	...
	{	switch(data.funCode)
		{
			case 3:
			{
				...
				AANotifyResult r;
				r.type = evt;
				r.funID = data.funCode;
				r= pDlg->AARobotProcessResult[0];
				result.push_back(r);
			}
			break;
			case 4:
...
LRESULT CHSFoxxDlg::OnMsgNotify(WPARAM wParam, LPARAM lParam)
{
	...
	case STARTAA:
		...
		if (pData->funCode==3 /*|| pData->visionid==2*/)//序号为1，2的视觉对应1，2好机器人
		{
			AARobotProcessResult[0].result = INVALID_RESULT;
			AARobotProcessResult[0].isfinsh = 0;
			for (int i = 0;i<3;i++)
			{
				AARobotProcessResult[0].tha[i] = 0.0;
				AARobotProcessResult[0].thb[i] = 0.0;
			}
			AARobotProcessParam[0]= 1; //后续要写配置: 功能好对应不同的盒子索引，或者是通道索引
			AARobotProcessParam[1]= 2;
		...
		else if (pData->funCode==4 /*|| pData->visionid==4*/)
		{
			AARobotProcessResult[1].result = INVALID_RESULT;
			AARobotProcessResult[1].isfinsh = 0;
			for (int i = 0;i<3;i++)
			{
				AARobotProcessResult[1].tha[i] = 0.0;
				AARobotProcessResult[1].thb[i] = 0.0;
			}
			AARobotProcessParam[2]= 3;
			AARobotProcessParam[3]= 4;
			SetEvent(dlgAARobotProcessEvent[1]);
		}
	...
	case RESETAA:
		...
		break;
	case DAA:
		if (pData->funCode==3 /* || pData->visionid==5 */)//通道索引3 上有两个芯片凹槽
		{
			AARobotProcessResult[0].result = INVALID_RESULT;
			AARobotProcessResult[0].isfinsh = 0;
			for (int i = 0;i<3;i++)
		...
		break;
	case LIGHT:
		if (pData->funCode==1 /* || pData->visionid==5 */)//通道索引3 上有两个芯片凹槽
		{
			AARobotProcessResult[2].result = INVALID_RESULT;
			AARobotProcessParam[4]= 1;
			AARobotProcessParam[5]= 2;
			SetEvent(dlgAARobotProcessEvent[2]);
		}
		else if (pData->funCode==2/* || pData->visionid==4*/)
		{
			AARobotProcessResult[3].result = INVALID_RESULT;
			AARobotProcessParam[6]= 3;
			AARobotProcessParam[7]= 4;
...
int WINAPI CHSFoxxDlg::AARobotProcess1(void* param)//先启动线程
{
	...
	while (!pWork->m_dlgExit)
	{
		...
		int visionAID = -1;
		int visionBID = -1;
		if (1 == g_AAParam.IsRobot)//判断运动机构是否是机器人
		{
			//走到AARobotProcess1,标识左工位的机器人,配置时把左工位可以动的机器人IP设置到AAROBOT1
			robotID = pWork->GetRobotIDByIndex(AAROBOT1);//这个AAROBOT1可以在配置文件里配置
			//调整之前先把机器人移动到初始位置
			pWork->AARobotProcessResult[0].result =  pWork->GoInitPosRobot(robotID,axisString,AAINDEX2,NULL);//这个初始位置是调整前机器人初始位置，不是接镜头的初始位置
			visionIndexA = pWork->AARobotProcessParam[0] ;
			visionIndexB = pWork->AARobotProcessParam[1] ;
			//规定视觉序号为1，2，3，4;发序号1:表示启动机器人1,2,视觉通道1,2, 但是视觉通道1是都对应机器人1不知道，那就改掉名字，使得
			//机器人1对应camera_1,cameram_3,机器人2对应camera_2,cameram_4
			visionAID = pWork->GetVisionIDByIndex(visionIndexA,cameraName);		
			ret = pWork->__OpenAAVision(cameraName,pWork->m_playvision[visionAID]); //打开视频函数:相机名字,视频窗口id 
			visionBID = pWork->GetVisionIDByIndex(visionIndexB,cameraName);
			ret |= pWork->__OpenAAVision(cameraName,pWork->m_playvision[visionBID]);
			if (0 == pWork->AARobotProcessResult[0].result && ret == DT_ERROR_OK)
			{

				pWork->AARobotProcessResult[0].result = pWork->StartDoubleAA(robotID,visionAID,visionBID);			
				msg("线程1,视觉通道:%d,机器人id:%d,视觉id:%d,调整结果:%d",visionIndex,robotID,visionID,pWork->AARobotProcessResult[0]);
			}
			else
			{
				pWork->AARobotProcessResult[0].result = 0;
				pWork->AARobotProcessResult[0].isfinsh = 0;
		...
		{
			//运动控制卡的双摄AA,只需要获取两张图，进行比较，然后求取u，v,w
			int visionIndexA = pWork->AARobotProcessParam[0] ;//运动控制卡的话，传入3是什么意思
			int visionIndexB = pWork->AARobotProcessParam[1] ;//运动控制卡的话，传入3是什么意思
			int visionIDA = pWork->GetVisionIDByIndex(visionIndexA,cameraName);
			int visionIDB = pWork->GetVisionIDByIndex(visionIndexB,cameraName);
			//ret = pWork->__OpenAAVision(cameraName,pWork->m_playvision[visionIDA]);//测试的时候是否需要打开视频
			//ret |= pWork->__OpenAAVision(cameraName,pWork->m_playvision[visionIDB]);
			ret = 1;
			if (ret == DT_ERROR_OK)
			{
				pWork->AARobotProcessResult[0] = pWork->NewStartDoubleAA(visionIDA,visionIDB);	//得到两个模组的偏差值后，传出去，若ret异常传错	
				msg("线程1,视觉A:%d,视觉B:%d",visionIDA,visionIDB);
			}
			else
			{
				pWork->AARobotProcessResult[0].result = 0;
				pWork->AARobotProcessResult[0].isfinsh = 0;
				msg("线程1,视觉A:%d,视觉B:%d 回初始位置失败!",visionIDA,visionIDB);
			}
		}
...
int WINAPI CHSFoxxDlg::AARobotProcess2(void* param)
{
	...
	while (!pWork->m_dlgExit)
	{
		...
		{
			//走到AARobotProcess1,标识左工位的机器人,配置时把左工位可以动的机器人IP设置到AAROBOT1
			robotID = pWork->GetRobotIDByIndex(AAROBOT2);//这个AAROBOT1可以在配置文件里配置
			//调整之前先把机器人移动到初始位置
			pWork->AARobotProcessResult[0].result =  pWork->GoInitPosRobot(robotID,axisString,AAINDEX2,NULL);//这个初始位置是调整前机器人初始位置，不是接镜头的初始位置
			visionIndexA = pWork->AARobotProcessParam[2] ;
			visionIndexB = pWork->AARobotProcessParam[3] ;

			visionAID = pWork->GetVisionIDByIndex(visionIndexA,cameraName);		
			ret = pWork->__OpenAAVision(cameraName,pWork->m_playvision[visionAID]); //打开视频函数:相机名字,视频窗口id 
			visionBID = pWork->GetVisionIDByIndex(visionIndexB,cameraName);
			ret |= pWork->__OpenAAVision(cameraName,pWork->m_playvision[visionBID]);
			if (0 == pWork->AARobotProcessResult[1].result && ret == DT_ERROR_OK)
			{

				pWork->AARobotProcessResult[1].result = pWork->StartDoubleAA(robotID,visionAID,visionBID);			
				msg("线程2,视觉通道:%d,机器人id:%d,视觉id:%d,调整结果:%d",visionIndex,robotID,visionID,pWork->AARobotProcessResult[1]);
			}
			else
			{
				pWork->AARobotProcessResult[1].result = 0;
				pWork->AARobotProcessResult[1].isfinsh = 0;
		...
		{
			//运动控制卡的双摄AA,只需要获取两张图，进行比较，然后求取u，v,w
			int visionIndexA = pWork->AARobotProcessParam[2] ;//运动控制卡的话，传入3是什么意思
			int visionIndexB = pWork->AARobotProcessParam[3] ;//运动控制卡的话，传入3是什么意思
			int visionIDA = pWork->GetVisionIDByIndex(visionIndexA,cameraName);
			int visionIDB = pWork->GetVisionIDByIndex(visionIndexB,cameraName);
			ret = pWork->__OpenAAVision(cameraName,pWork->m_playvision[visionIDA]);//测试的时候是否需要打开视频
			ret |= pWork->__OpenAAVision(cameraName,pWork->m_playvision[visionIDB]);
			if (ret == DT_ERROR_OK)
			{
				pWork->AARobotProcessResult[1] = pWork->NewStartDoubleAA(visionIDA,visionIDB);	//得到两个模组的偏差值后，传出去，若ret异常传错	
...
int WINAPI CHSFoxxDlg::AARobotProcess3(void* param)
{
	...
	while (!pWork->m_dlgExit)
	{
		...
		if (1 == g_AAParam.IsRobot)//判断运动机构是否是机器人
		{
			...
			//调整之前先把机器人移动到初始位置
			//pWork->AARobotProcessResult[0].result =  pWork->GoInitPosRobot(robotID,axisString,AAINDEX2,NULL);//这个初始位置是调整前机器人初始位置，不是接镜头的初始位置
			visionIndexA = pWork->AARobotProcessParam[4] ;
			visionIndexB = pWork->AARobotProcessParam[5] ;
			...
			if (ret == DT_ERROR_OK)
			{
				pWork->AARobotProcessResult[2].result = 1;
				msg("线程3,视觉ida:%d,视觉idb:%d,点亮:%d",visionAID,visionBID,ret);
			}
			else
			{
				pWork->AARobotProcessResult[2].result = 0;
				msg("线程3,视觉ida:%d,视觉idb:%d 点亮失败!",visionAID,visionBID);
			}
		}
		else
		{
			//运动控制卡的双摄AA,只需要获取两张图，进行比较，然后求取u，v,w
			int visionIndexA = pWork->AARobotProcessParam[4] ;//运动控制卡的话，传入3是什么意思
			int visionIndexB = pWork->AARobotProcessParam[5] ;//运动控制卡的话，传入3是什么意思
			int visionIDA = pWork->GetVisionIDByIndex(visionIndexA,cameraName);
			int visionIDB = pWork->GetVisionIDByIndex(visionIndexB,cameraName);
			ret = pWork->__OpenAAVision(cameraName,pWork->m_playvision[visionIDA]);//测试的时候是否需要打开视频
			ret |= pWork->__OpenAAVision(cameraName,pWork->m_playvision[visionIDB]);
...
int WINAPI CHSFoxxDlg::AARobotProcess4(void* param)
{
	...
	while (!pWork->m_dlgExit)
	{
		...
		if (1 == g_AAParam.IsRobot)//判断运动机构是否是机器人
		{
			visionIndexA = pWork->AARobotProcessParam[6] ;
			visionIndexB = pWork->AARobotProcessParam[7] ;
			visionAID = pWork->GetVisionIDByIndex(visionIndexA,cameraName);		
			ret = pWork->__OpenAAVision(cameraName,pWork->m_playvision[visionAID]); //打开视频函数:相机名字,视频窗口id 
			visionBID = pWork->GetVisionIDByIndex(visionIndexB,cameraName);
			ret |= pWork->__OpenAAVision(cameraName,pWork->m_playvision[visionBID]);
			if (ret == DT_ERROR_OK)
			{
				pWork->AARobotProcessResult[3].result = 1;
				msg("线程3,视觉ida:%d,视觉idb:%d,点亮:%d",visionAID,visionBID,ret);
			}
			else
			{
				pWork->AARobotProcessResult[3].result = 0;
				msg("线程3,视觉ida:%d,视觉idb:%d 点亮失败!",visionAID,visionBID);
			}
		}
		else
		{
			//运动控制卡的双摄AA,只需要获取两张图，进行比较，然后求取u，v,w
			int visionIndexA = pWork->AARobotProcessParam[6] ;//运动控制卡的话，传入3是什么意思
			int visionIndexB = pWork->AARobotProcessParam[7] ;//运动控制卡的话，传入3是什么意思
			int visionIDA = pWork->GetVisionIDByIndex(visionIndexA,cameraName);
			int visionIDB = pWork->GetVisionIDByIndex(visionIndexB,cameraName);
			ret = pWork->__OpenAAVision(cameraName,pWork->m_playvision[visionIDA]);//测试的时候是否需要打开视频
			ret |= pWork->__OpenAAVision(cameraName,pWork->m_playvision[visionIDB]);
...
int CHSFoxxDlg::__OpenAAVision(string visionName,CButton &bt)
{
	...
	if (!vision)
	{
		...
		HWND h = GetHWndByVisionId(vision->m_iDevIDA);//这个函数绑定视觉id跟窗口有问题，默认视觉id从0开始
		//测试窗口绑定：
		RECT videosize ;
		m_wndVideo[vision->m_iDevIDA].GetWindowRect(&videosize);
		int width = videosize.right - videosize.left;
		...
		msg("vision(%s)视频打开成功,前id为:%d,当前id为:%d !",visionName.c_str(),m_nVisionDevID,vision->m_iDevIDA);
		m_nVisionDevID = vision->m_iDevIDA;//
		//把下拉框中的选项选为当前vision;
		int nIndex = m_KitListCtrl.GetCurSel();
		int nCount = m_KitListCtrl.GetCount();
		CString str;
...
int CHSFoxxDlg::OpenAAVision(string visionName,CButton &bt)
{
	...
	//设置水平滚动条曝光,增益
	unsigned int value = vision->GetInitRegValue(AA_GRAIN);
	if (value <AAVISION_MAXGRAIN)
	{
		m_grain.SetPos(value);
	}
	value = vision->GetInitRegValue(AA_EXP);
	if (value < AAVISION_MAXEXPURE)
	{
		m_exp.SetPos(value);
	}
	...
	else//关闭
	{
		...
		{
			if (vision->m_iDevIDA == m_nVisionDevID)
			{
...
int CHSFoxxDlg::DobuleAATestReadPic(int robotID,int cameraID)
{
	...
	int ret = AA_OK;
	HObject m_pImage[8];
	HTuple TupleTemp,H_Width,H_Height,r,g,b;
	int iWidth = 4208,  Height = 3120;
	int index = 0;
	char * pBuf = NULL;
	byte *R= nullptr,*G= nullptr,*B= nullptr;
	bool flag = m_pImage[index].IsInitialized();
	if(flag)
	GetImageSize(m_pImage[index], &H_Width,  &H_Height);//1

	m_pImage[index].Clear();
	if(m_pImage[index].IsInitialized())
	GetImageSize(m_pImage[index], &H_Width,  &H_Height);//2
	if(m_pImage[index].IsInitialized())
	GenImageConst(&m_pImage[index],"byte",iWidth, Height);//产生空对象
	if(m_pImage[index].IsInitialized())
	GetImageSize(m_pImage[index], &H_Width,  &H_Height);//3

	GenImageConst(&m_pImage[index],"byte",iWidth*3, Height);//产生空对象
	m_pImage[index].Clear();
	ReadImage(&m_pImage[index],".\\111.JPG");
	if(m_pImage[index].IsInitialized())
		GetImageSize(m_pImage[index], &H_Width,  &H_Height);//3
	GetImagePointer3(m_pImage[index],&r,&g,&b, &TupleTemp, &H_Width,  &H_Height);//获取图像指针,类型,宽,高
	R = const_cast<byte*>((byte*)((int)(r)));
	G = const_cast<byte*>((byte*)((int)(g)));
	B = const_cast<byte*>((byte*)((int)(b)));

	WriteImage(m_pImage[index],"jpg",0,"1111.jpg");

	if(m_pImage[index].IsInitialized())
	GetImageSize(m_pImage[index], &H_Width,  &H_Height);//4
	...
	Mat img(Height,iWidth,CV_8UC1,pBuf);
	IAVO_ShowPicture("1",img);
	Mat img1(Height,iWidth,CV_8UC1,R);
	int a = img.channels();
	 a = img.depth();
	IAVO_ShowPicture("2",img1);
	HTuple HalWinId;
	long halconWinID;
	OpenWindow(0,0,512,512, 0, "visible", "", &HalWinId);
	halconWinID = (long)HalWinId.L();
	SetPart(halconWinID,0,0,Height,iWidth);


DispObj(m_pImage[index],halconWinID);
HalconCpp::CloseWindow(halconWinID);

	msg("请注意双摄靶图是不是符合要求!!!!!");
	
	msg("请在在.\\image目录下放入fix.jpg,mov.jpg两张图片！");
	int flag1 = ReadPictureMat(&movMat,".\\image\\mov.jpg",cameraID);
	if(flag1 != TRUE)
	{
	...
	//int a = IAV_ADD(1,2);
	//a= IAV_ADD(1.0f,1.2f);
	IAVO_ShowPicture("1",movMat);
	IAVO_SavePicture(".\\image\\111.jpg",movMat);
	movMat = Img_resize(movMat, 0.2);
	IAVO_SavePicture(".\\image\\112.jpg",movMat);
...
void CHSFoxxDlg::OnBnClickedReadpicTest()
{
	...
	if (g_AAParam.AATYPE == DOUBLEAA)
	{
		...
		ret=  DobuleAATestReadPic(m_nPiDevID,m_nVisionDevID);
		return ;
	...
	double distanceToOriginal[10]={0};
	int  first_z_number = AdjustZvalueReadPic(m_nPiDevID,m_nVisionDevID);
	if (first_z_number<0)
	{
		return ;
	}
	//二：然后需再调整U/V轴
	//int  adjust_uv_number = AdjustUVvalueReadPic(m_nPiDevID,m_nVisionDevID);
	//if (adjust_uv_number<0)
	{
		return ;
	}
...
CAAVision *vision = NULL;
	...
	*/
	...
	//测试获取spi坐标系坐标
	/*CAARobot *robot = FindRobotByID(m_nPiDevID);
	if (robot != NULL)
	{
		const char* axisArr = "R";
	//	double posArr[PI_AXIS_MAXNUM] = {0.0};
		double posArr =1.0;
		int result = robot->GetPivotPointCoordinates(robot->GetAARobotID(),axisArr,&posArr);
	}*/
	CAARobot *robot = FindRobotByID(m_nPiDevID);
	if (robot == NULL) return ;

	char buffer[LOG_MAX_BUFF] ;
	int bufsize = LOG_MAX_BUFF -1;
	int result = robot->GetKLTCoordinates(m_nPiDevID, AA_TOOL2, AA_TOOL2, buffer, bufsize);
	

	//测试获取速度，设置速度
	const char *axises = "X Y Z U V W";
	double cvel= 0.0;
	result = robot->GetSystemVelocity(m_nPiDevID,&cvel);

	double vel = (cvel+1);
	result = robot->SetSystemVelocity(m_nPiDevID,vel);
	result = robot->SetSystemVelocity(m_nPiDevID,cvel);


	///暂时无法找到方法：冻结一个坐标系，只知道创建一个坐标系,激活一个坐标系,如果有两个坐标系，若救活一个坐标系,另外一个坐标系会自动冻结，
	//所以可以这样:原来有一个坐标系,再创建一个坐标系,然后激活后面这个坐标系,再删除前面那个坐标系
	double ToolCoordinateValue[PI_AXIS_MAXNUM] = {152.8,0,-15.3,0.0,0.0,180};
	//const char* szNameOfCoordSystem = "TOOL2";
	string findstr = "KST=";
	string tool;
	string newtool;
	...
	//查询所有激活的坐标系，按类型排列
	result = robot->QueryActiveCoordinates(m_nPiDevID, "",buffer, bufsize);//查询所有激活的坐标系，
	if (result == TRUE)
	{
		string str = buffer;
		std::size_t found = str.find(findstr);
		if (found!=std::string::npos)  
		{
			std::size_t found2 = str.find(' ',found+findstr.length()-1);
			if (found2!=std::string::npos) 
			{
				tool = str.substr(found+findstr.length(),found2-found-findstr.length());
			}
		}
	}
	if (tool.length()==0)
	...
	{
		result = robot->GetKLTCoordinates(m_nPiDevID, tool.c_str(), tool.c_str(), buffer, bufsize);
		//解析坐标X= Y= Z= U= V= W=
		if (result == TRUE)
		{
			string str = buffer;
			...
			string fstr[PI_AXIS_MAXNUM] = {"X=","Y=","Z=","U=","V=","W="};

			string aixsvalue;
			for (int i= 0;i<PI_AXIS_MAXNUM;i++)
			{	
				std::size_t found = str.find(fstr[i]);
				if (found!=std::string::npos)  
				{
					std::size_t found2 = str.find('\t',found+fstr[i].length()-1);
					if (found2!=std::string::npos) 
					{
						aixsvalue = str.substr(found+fstr[i].length(),found2-found-fstr[i].length());
						if (aixsvalue.length()==0)
						{
							return;
						}
						ToolCoordinateValue[i]= atof(const_cast<const char *>(aixsvalue.c_str()));
						if (0 == i)
						{
							ToolCoordinateValue[i]+=0.1;
						}
					}
				}
			}
	...
	//比较tool名称
	if (!tool.compare(AA_TOOL))//如果激活的为"TOOL"
	{
		newtool = AA_TOOL2;
	}
	else if (!tool.compare(AA_TOOL2))
	{
		newtool = AA_TOOL;
	}
	else
	...
	//创建一个新的坐标系并激活
	result = robot->DeleteOneCoordinate(m_nPiDevID,newtool.c_str());//可以先删除
	result = robot->DefineKSTCoordinate(m_nPiDevID, newtool.c_str(), axises,   ToolCoordinateValue);
	result = robot->ActiveOneCoordinate(m_nPiDevID,newtool.c_str());

	//result = robot->GetKLTCoordinates(m_nPiDevID, szStartCoordSystem, szEndCoordSystem, buffer, bufsize);	
	//result = robot->GetKLTCoordinates(m_nPiDevID, szNameOfCoordSystem, szNameOfCoordSystem, buffer, bufsize);

	//删除旧tool
	result = robot->DeleteOneCoordinate(m_nPiDevID,tool.c_str());
...
void CHSFoxxDlg::OnHScroll(UINT nSBCode, UINT nPos, CScrollBar* pScrollBar)
{
	...
	if (pScrollBar->GetDlgCtrlID() == IDC_GRAIN)
	{
		...
		result = SetSensorRegValue(m_nVisionDevID,AA_GRAIN,value);//0x00C6);
		if (DT_ERROR_OK != result)
		{
			msg("设置增益失败:%x ",value);
		}
		value = GetSensorRegValue(m_nVisionDevID,AA_GRAIN);
		msg("当前增益:%x ",value);
	}
	else if(pScrollBar->GetDlgCtrlID() == IDC_EXP)
	{
		value = m_exp.GetPos();
		result = SetSensorRegValue(m_nVisionDevID,AA_EXP,value);//0x00C6);
		if (DT_ERROR_OK != result)
		{
			msg("设置曝光失败:%x ",value);
		}
		value = GetSensorRegValue(m_nVisionDevID,AA_EXP);
		msg("当前曝光:%x ",value);
	}
...
CAARobot* CHSFoxxDlg::FindRobotByID(int robotID)
{
	CAARobot *robot = NULL;
	map<string,CAARobot*>::iterator it;
	//在map中找当前视觉id
	...
	if (robot == NULL )
	{
		msg("当robotid:%d对应机器人不存在,视觉id:%d",robotID);
	}
...
void CHSFoxxDlg::OnBnClickedSpinCenter()
{
	if (m_spincenter.GetCheck() == TRUE)//调整
	{
...
int CHSFoxxDlg::AdjustSpinCenter(int robotID,int whichaxis,double value)//此方法中没有加入设置正负限位以及保存参数到永久存储里，调整后还是要打开PI软件进行软件限制
{
	///暂时无法找到方法：冻结一个坐标系，只知道创建一个坐标系,激活一个坐标系,如果有两个坐标系，若救活一个坐标系,另外一个坐标系会自动冻结，
	//所以可以这样:原来有一个坐标系,再创建一个坐标系,然后激活后面这个坐标系,再删除前面那个坐标系
	...
	CAARobot *robot = FindRobotByID(robotID);
	if (robotID <0 || robot == NULL) 
	{
		msg("机器人（id:%d）不存在!",robotID);
		return result;
	}
	//查询所有激活的坐标系，按类型排列
	result = robot->QueryActiveCoordinates(robotID, "",buffer, bufsize);//查询所有激活的坐标系，
	if (result == TRUE)
	{
		string str = buffer;
		std::size_t found = str.find(findstr);
	...
	{
		result = robot->GetKLTCoordinates(robotID, tool.c_str(), tool.c_str(), buffer, bufsize);	
		if (result == TRUE)
		{
			string str = buffer;
			string fstr[PI_AXIS_MAXNUM] = {"X=","Y=","Z=","U=","V=","W="};//解析坐标X= Y= Z= U= V= W=
			string aixsvalue;
			for (int i= 0;i<PI_AXIS_MAXNUM;i++)
			{	
				std::size_t found = str.find(fstr[i]);
				if (found!=std::string::npos)  
				{
					...
					if (!fstr[i].compare("W="))
					{
						found2 = str.find('\n',found+fstr[i].length()-1);
					}else
					{
						found2 = str.find('\t',found+fstr[i].length()-1);
					}
	...
	//创建一个新的坐标系并激活
	result = robot->DeleteOneCoordinate(robotID,newtool.c_str());//可以先删除
	result = robot->DefineKSTCoordinate(robotID, newtool.c_str(), axises,   ToolCoordinateValue);
	result = robot->ActiveOneCoordinate(robotID,newtool.c_str());

	//删除旧tool
	result = robot->DeleteOneCoordinate(robotID,tool.c_str());

	//获取新的旋转中心
	result = robot->GetKLTCoordinates(robotID, newtool.c_str(), newtool.c_str(), buffer, bufsize);
	msg(buffer);
...
map<string ,CAAVision*>& CHSFoxxDlg::GetVisionMap()
{
	return mapVision;
}

CHSFoxxDlg* CHSFoxxDlg::GetInstance()
{
	static CHSFoxxDlg* pDlg = NULL;
	pDlg = (CHSFoxxDlg*)AfxGetMainWnd();
	return pDlg;
...
void CHSFoxxDlg::OnBnClickedDrawcenterpoint()
{
	// TODO: 在此添加控件通知处理程序代码
	CButton* pBtn = (CButton*)GetDlgItem(IDC_DRAWCENTERPOINT);
	int state = pBtn->GetCheck();
	if (state == TRUE)
	{
		//CAAVision::SetPaintFlag(TRUE);
