...
public:
	...
	int GetSensorRegValue(int type);//得到增益,曝光
	int SetSensorRegValue(int type,USHORT value);//得到增益,曝光

	UINT GetInitRegValue(int type);

	int InitCamera();//打开视频要初始化相机,抓图也要开视频
	int UinitCamera();//打开视频要初始化相机,抓图也要开视频
	
	int OpenVisionVideo(HWND hWnd,bool windtype,int windwid, int windheight);
	int CloseVisionVideo();

	int ChangeVideoWind(HWND hWnd,bool windtype,int windwid, int windheight);//双击窗口通知视觉
	...
	int m_videowindwid;   //视频显示窗口大小，便于测试halocn
	int m_videowindheight;
	bool m_videowintype;  //是否是大窗口
	bool m_firstopenvideo;   //是否第一次打开视频
	CRITICAL_SECTION	m_windcsLock;
