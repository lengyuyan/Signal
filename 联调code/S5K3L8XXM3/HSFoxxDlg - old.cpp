...
int CHSFoxxDlg::AdjustZvalue(int robotID,int cameraID)
{
	...
	//调整次数小于等于z_adjustnum，或者(调整后的z小于zadjust_zthr且distanceToOriginal[temp_number]<0.02，他们有一个成立就继续
	while(!(( fabs(AAadjust_z)<zadjust_zthr && distanceToOriginal[temp_number]< 0.02)||(temp_number<=z_adjustnum)))
	{
		//算法: 获取当前  上下共3张图片，然后计算这3张图的判定值，若发现还要调整z,z不在精度范围内，
		//那么还需要调整，直到精度符合要求或者超出次数限制

	    //step1:保存当前帧的图片以及该图像正负向调整的像.......	
		int Ori_flag = GetPictureMat(&Ori_PictureMat,cameraID);
		if(Ori_flag != TRUE)
		{
			msg("(robotid:%d,visonid:%d)获取原始图片失败,",robotID,cameraID);
		...
		if(Plus_flag != TRUE)
		{
			msg("获取距离为:%.4f图片失败,调整次数:%d",distanceToOriginal[temp_number],temp_number);
			return AA_GET_PICTIRE_FAIL;
		}
...
int CHSFoxxDlg::StartDoubleAA(int robotid,int visonidfixID,int visonidmovID)
{
	...
	while (true)
	{
		...
		if (count >=countth)
		{
			msg("需要调整范围:%.6lf,v:%.6lf,w:%.6lf,调整超出限制10次!",movpos[0],movpos[1],movpos[2]);
			break;
		}
		count++;
