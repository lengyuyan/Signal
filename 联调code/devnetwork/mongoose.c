...
#pragma comment (lib,"Advapi32.lib") ////新增  gpz 报错信息可知注册表函数缺少相关库 是否我的系统是win10造成的
/*
...
MG_INTERNAL size_t mg_handle_chunked(struct mg_connection *nc,
  ...
  if (i > body_len) {
    ...
    if (nc->flags & MG_F_DELETE_CHUNK) {
      ...
      hm->body.len = (size_t)pd->chunk.body_len = 0;
    }
...
void mg_register_http_endpoint(struct mg_connection *nc, const char *uri_path,
  ...
  new_ep->name = _strdup(uri_path);//strdup --->_strdup
