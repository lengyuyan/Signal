...
HV_Lap getAllLaplace(Mat I, int edge_size, int error)
{
	...
	for (int i = 0; i < x_edge.size(); i++){
		if ((abs(x_edge[i].x - center_x.x) < error) && (abs(x_edge[i].y - center_x.y) < error)){		
			Loc_center_x = x_edge[i];
		}
		else if ((x_edge[i].x < center_x.x - error) && (abs(x_edge[i].y - center_x.y) < error)){
			Loc_left = x_edge[i];
		}
		else if ((x_edge[i].x > center_x.x - error) && (abs(x_edge[i].y - center_x.y) < error)){
			Loc_right = x_edge[i];
	...
	for (int i = 0; i < y_edge.size();i++){
		if ((abs(y_edge[i].y - center_y.y) < error) && (abs(y_edge[i].x - center_y.x) < error)){
			Loc_center_y = y_edge[i];
		}
		else if ((y_edge[i].y < center_y.y - error) && (abs(y_edge[i].x - center_y.x) < error)){
			Loc_top = y_edge[i];
		}
		else if ((y_edge[i].y > center_y.y - error) && (abs(y_edge[i].x - center_y.x) < error)){
			Loc_bottom = y_edge[i];
	...
	cout << "Laplace Value Center_x:   " << str.horizontal_lap[1] << endl;
	cout << "Laplace Value Center_y:   " << str.vertical_lap[1] << endl;
	cout << "Laplace Value Left:       " << str.horizontal_lap[0] << endl;
	cout << "Laplace Value Right:      " << str.horizontal_lap[2] << endl;
	cout << "Laplace Value Top:        " << str.vertical_lap[0] << endl;
	cout << "Laplace Value Bottom:     " << str.vertical_lap[2] << endl;
...
double laplace(Mat I, Point Loc, int edge_size)
{
	...
	double lap = sum_lap / ((2 * edge_size - 1)*(2 * edge_size - 1));
	return lap;
}
/*
double laplace(Mat I, Point Loc, int edge_size)
