#include <opencv2\opencv.hpp>
#include <iostream>
#include <string>
#include <math.h>
#include <sstream>

using namespace cv;
using namespace std;

typedef struct {
	double x_angle;
	double y_angle;
	double deltaZ;
	double time_Edge;
	double time_Lap;
	vector<double> horizontal_lap;
	vector<double> vertical_lap;
}State; // get the current state of the initial image including u, v and z
typedef struct {
	double time;
	double time_Lap;
	double time_Edge;
	double adjust_z;
	double adjust_u;
	double adjust_v;
	vector<double> horizontal_lap;
	vector<double> vertical_lap;
}Running; 
State getState(Mat img_ori, Mat img_p, Mat img_m, double deltaZ_p, double deltaZ_m, double pixel_size);
Mat Img_resize(Mat I, double scale);
string ConvertToString(double value);
/*
%Main program
%============================================
%Author:Ocean
%Date:2017.4.10
%============================================
%Nargin Set
% ori_num: the number of the original image, related to the "img_ori";
% p_num: the number of the positive image, related to the "img_p";
% m_num: the number of the minus image, related to the "img_m";
% pixel size: set as 1.12μm;
% std_error: set as 0.03;
%--------------------------------------------
%Output
% Output by getState();
%============================================
*/
Running Run(vector<string> filename, double deltaZ_p, double deltaZ_m){
	//依次读入p，ori，m，注意命名规则！
	double time = static_cast<double>(getTickCount());
	//参数输入
	double scale = 0.2;
	double pixel_size = 0.00112 / scale;
	double std_error = 0.03;
	Mat img_ori = imread(filename[1]);
	Mat img_p = imread(filename[0]);
	Mat img_m = imread(filename[2]);
	img_ori = Img_resize(img_ori, scale);
	img_p = Img_resize(img_p, scale);
	img_m = Img_resize(img_m, scale);
	State state;
	state = getState(img_ori,img_p,img_m,deltaZ_p,deltaZ_m,pixel_size);
	double adjust_z = state.deltaZ;
	double adjust_u = state.x_angle * -1;
	double adjust_v = state.y_angle * -1;
	time = ((double)getTickCount() - time) / getTickFrequency();
	Running Running;
	Running.time = time;
	Running.adjust_z = adjust_z;
	Running.adjust_u = adjust_u;
	Running.adjust_v = adjust_v;
	Running.time_Edge = state.time_Edge;
	Running.time_Lap = state.time_Lap;
	Running.horizontal_lap = state.horizontal_lap;
	Running.vertical_lap = state.vertical_lap;
	return Running;
}
Mat Img_resize(Mat I, double scale){
	if (I.channels() == 3){
		cvtColor(I, I, CV_RGB2GRAY);
	}
	int height = I.rows * scale;
	int width = I.cols * scale;
	resize(I, I, Size(width, height));
	I.convertTo(I, CV_32F);
	I = I / 255;
	return I;
}
