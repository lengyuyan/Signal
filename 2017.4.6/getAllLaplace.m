...
for k = 1:3
    ...
    temp_h = medfilt2(temp_h, [3,3]);
    temp_v = medfilt2(temp_v, [3,3]);
    horizontal_lap(k) = laplace(temp_h);
    vertical_lap(k) = laplace(temp_v);
end

end
function lap = laplace(edge)

[m, n] =size(edge);
lap = edge(3:end,3:end)+edge(3:end,2:end-1)+edge(3:end,1:end-2)+edge(2:end-1,1:end-2)...
    +edge(1:end-2,1:end-2)+edge(1:end-2,2:end-1)+edge(1:end-2,3:end)+edge(2:end-1,3:end);
lap = 8*edge(2:end-1,2:end-1) - lap;
lap = sum(lap(:).^2);
lap = lap / ((m-2)*(n-2));
end